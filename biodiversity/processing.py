...
def observations_by_park(observations: pd.DataFrame) -> pd.DataFrame:
    if (
        "park_name" not in observations.columns
        or "observations" not in observations.columns
    ):
    ...
    park_counts = park_counts.sort_values("observations", ascending=False).reset_index(
        drop=True
    )
    return park_counts
...
def preprocess(
    species: pd.DataFrame, observations: pd.DataFrame
    ...
    if "park_name" in obs.columns:
        obs["park_name"] = obs["park_name"].str.replace(
            " National Park", "", regex=True
        )
    ...
    if (
        "scientific_name" in obs.columns
        and "observations" in obs.columns
        and "scientific_name" in sp.columns
    ):
